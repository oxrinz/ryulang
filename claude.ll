...
@sym_cuInit = private constant [8 x i8] c"cuInit\00"
@sym_cuDeviceGet = private constant [12 x i8] c"cuDeviceGet\00"
@sym_cuCtxCreate = private constant [14 x i8] c"cuCtxCreate_v2\00"
@sym_cuModuleLoadData = private constant [16 x i8] c"cuModuleLoadData\00"
@sym_cuModuleGetFunction = private constant [20 x i8] c"cuModuleGetFunction\00"
@sym_cuMemAlloc = private constant [13 x i8] c"cuMemAlloc_v2\00"
@sym_cuMemcpyHtoD = private constant [15 x i8] c"cuMemcpyHtoD_v2\00"
@sym_cuMemcpyDtoH = private constant [15 x i8] c"cuMemcpyDtoH_v2\00"
...
@msg_error = private constant [27 x i8] c"Error at step with code: \00"
@msg_file_error = private constant [32 x i8] c"Failed to open or read PTX file\0A\00"
@msg_success = private constant [30 x i8] c"Kernel executed successfully!\0A\00"
@msg_result = private constant [17 x i8] c"Kernel results:\0A\00"
@msg_float = private constant [8 x i8] c"000.00 \00"
...
setup_params:
    ...
    %d_input_ptr_val = ptrtoint i64* @d_input to i64*
    store i64* %d_input_ptr_val, i64** %params_0_ptr
    
    %params_1_ptr = getelementptr i64*, i64** %params_ptr, i64 1
    %d_output_ptr_val = ptrtoint i64* @d_output to i64*
    store i64* %d_output_ptr_val, i64** %params_1_ptr
    
    %params_2_ptr = getelementptr i64*, i64** %params_ptr, i64 2
    %d_param_n_ptr_val = ptrtoint i64* @d_param_n to i64*
    store i64* %d_param_n_ptr_val, i64** %params_2_ptr
...
launch_kernel:
    ...
    %cuLaunchKernel_result = call i32 %cuLaunchKernel_typed(
        ...
        i32 1,                    ; gridDimY
        i32 1,                    ; gridDimZ
        i32 4,                    ; blockDimX (4 threads)
        i32 1,                    ; blockDimY
        i32 1,                    ; blockDimZ
        i64 0,                    ; sharedMemBytes
        i8* null,                 ; hStream
        i8* null,                 ; extra
        i8** %params_cast         ; kernelParams
    )
...
; Helper function to format a 2-digit number at given buffer position
define void @format_digit(i8* %buffer, i32 %value) {
entry:
    ; Divide by 10 to get tens and ones digits
    %div = sdiv i32 %value, 10
    %rem = srem i32 %value, 10
    
    ; Convert to ASCII
    %tens_ascii = add i32 %div, 48  ; '0' is ASCII 48
    %ones_ascii = add i32 %rem, 48
    
    ; Store in buffer
    %tens_ptr = getelementptr i8, i8* %buffer, i64 0
    %ones_ptr = getelementptr i8, i8* %buffer, i64 1
