...
pub fn addStage(stage: Stage) !void {
    ...
    var cloned = std.array_list.Managed(*rir.RIROP).init(allocator);
...
fn prepareStages() anyerror![]const u8 {
    ...
    var configs = std.array_list.Managed(Config).init(allocator);
    defer configs.deinit();
    for (stages.items, 0..) |stage, stage_index| {
        var nodes = std.AutoHashMap(usize, Node).init(arena_allocator);
        defer nodes.deinit();
        var edges = std.array_list.Managed(Edge).init(arena_allocator);
        defer edges.deinit();
        ...
        var node_array = std.array_list.Managed(SerializableNode).init(allocator);
        ...
        var edge_array = std.array_list.Managed(SerializableEdge).init(allocator);
...
pub fn sendToDashboard() anyerror!void {
    ...
    _ = try req.sendBody(@constCast(payload));
}
